...
impl Server for GrpcService {
    const NAME: &'static str = "grpc-service-rs";
    async fn run(&self, mut event_rx: impl Stream<Item = ServiceEvent> + Unpin) {
        ...
        while let Some(service_event) = event_rx.next().await {
            let service_control_result = match service_event.service_control() {
